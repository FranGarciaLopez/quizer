...
class UserQuestions:
    ...
    def post(self, user_id, question_id):
        sql_statement = "INSERT INTO user_questions (user_id, question_id) VALUES ('{0}','{1}')"
        sql_statement = sql_statement.format(user_id, question_id)

        response = self.conn.engine.execute(sql_statement)
        return Response_Parser.post(response)
        
    def get_all(self, user_id):
        sql_statement = "SELECT * FROM user_questions where user_id = '{0}'"
    ...
    def get_one(self, user_id, question_id):
        sql_statement = "SELECT * FROM user_questions WHERE user_id = '{0}' and question_id = '{1}'"
        sql_statement = sql_statement.format(user_id, question_id)
    ...
    def delete(self, user_id, question_id):
        sql_statement = "DELETE FROM user_questions WHERE user_id = '{0}' and question_id = '{1}'"
        sql_statement = sql_statement.format(user_id, question_id)
