...
cd "../../api/backup/"
mkdir -p ./logs
echo "  - Installing postgres..."             && sudo apt-get update && sudo apt-get install postgresql-client                  >> ./logs/postgres_logs.log
echo "  - Creating database backup for tests" && psql postgresql://postgres:changeme@localhost:5432 -f tfg-db.sql           >> ./logs/postgres_logs.log
