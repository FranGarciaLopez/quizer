...
describe('Tests', function() {
    ...
    it(when+' if the user is in /admin/paths/id/tests/id/questions and clicks in "Add question" button, it should redirect to "/admin/paths/id/tests/id/questions/new" page', function(browser) {
        browser
            .assert.urlEquals('http://localhost:3001/admin/paths/2/tests/20/questions')
            .click('.float-right')
            .assert.urlEquals('http://localhost:3001/admin/paths/2/tests/20/questions/new')
            .pause(1000)
    });
    ...
    it(when+' if the user has created a new question from a test and wants to edit it then he must go to "/admin/paths/id/tests/id/question/id" URL and edit a field from the form then he should be redirected to "/paths/id/tests/id/questions" with the edited data', function(browser) {
        browser
    ...
    it(when+' if the user wants to delete a question from a path and clicks the delete button then it should be deleted', function(browser) {
        browser
            .url('http://localhost:3001/admin/paths/2/tests/20/questions')
