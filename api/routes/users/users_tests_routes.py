...
@app.route('/user/<int:user_id>/paths/<int:path_id>/tests', methods=['GET'])
@cross_origin()
def users_tests_get(user_id, path_id):
    return UserTests(db).get_all(user_id, path_id)
...
@app.route('/users/<int:user_id>/tests/<int:test_id>', methods=['PUT'])
