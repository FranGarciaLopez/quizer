...
class Tests_Questions:
    ...
    def get_all_test_questions(self, path_id, test_id):
        sql_statement = """
            select q.id, q."text", q.answers from questions q, tests t
            where t.path_id = {0} and t.id = {1} and q.test_id = t.id
            group by q.id, q."text", q.answers order by id asc""".format(path_id, test_id)
    ...
    def get_one_test_question(self, path_id, test_id, question_id):
        sql_statement = """
            ...
            group by q.id, q."text", q.answers order by id asc""".format(path_id, test_id, question_id)
